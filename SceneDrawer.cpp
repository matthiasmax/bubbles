...
extern XnBool g_bGame;   //Matthias
...
//		printf("Initialized depth texture: width = %d, height = %d\n", texWidth, texHeight);
	...
	// Wenn ich das richtig verstehe, zeigt pDepth auf die Liste aller Pixel
	// und pLabel zeigt auf die Liste der Pixel von erkannten Nutzern
	pDepth = dmd.Data();
	if (g_bDrawPixels)
	{
		XnUInt32 nIndex = 0;
			...
			for (nX=0; nX < g_nXRes; nX++, nIndex++)
			{
				...
				if (g_bDrawBackground || *pLabels != 0)
				{
					...
					if (nValue != 0)
					{
						...
						pDestImage[0] = nHistValue * Colors[nColorID][0];    //rot
						pDestImage[1] = nHistValue * Colors[nColorID][1];    //gruen
						pDestImage[2] = nHistValue * Colors[nColorID][2];    //blau
					}
				}
